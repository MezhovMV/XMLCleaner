...
int main(int argc, char* argv[])
{
    ...
    while (true) {
        ...
        if (!fs::is_directory(path) && Found(path,rx)) {
            fs::path file = path;

            if (!IsXml(file)) {
                log << file.filename() << " is not XML" << std::endl;
            }
            if (IsXml(file) && (!IsProject(file))) {
                log << file.filename() << " is not VsProject" << std::endl;
            }
            if (IsXml(file) && IsProject(file)) {
                CleanWin32(file);
                log << file.filename() << " is cleaned" << std::endl;
            }
